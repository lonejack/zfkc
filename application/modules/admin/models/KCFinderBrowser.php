<?php
class Admin_Model_KCFinderBrowser extends Admin_Model_KCFinderUploader {
	const		KC_DIE = 0;
	const		KC_OK  = 1;
	...
	protected $_headerAnswer = array();
	protected $_answer = array();
	
	public function getHeader()
	{
		return $this->_headerAnswer;
	}
	...
	public function __construct(Zend_Config $config, $options) {
	...
	public function action() {
		...
		if ($this->config['disabled']) {
			...
			$this->backMsg($message);
				...
				$this->_headerAnswer[] = "Content-Type: text/plain; charset={$this->charset}";
				//header("Content-Type: text/plain; charset={$this->charset}");
				$this->_answer[] = json_encode(array('error' => $message));
				return self::KC_DIE;
		...
		if ($act == "browser") {
			...
			$this->_headerAnswer[] ="Content-Type: text/html; charset={$this->charset}";
	...
	protected function output($data=null, $template=null) {
		...
		$template_dir = realpath(dirname(__FILE__)) ."/Kc_tpl";
		$template_file = $template_dir."/tpl_$template.php";
		...
		if (file_exists($template_file)) {
			...
			$eval .= "\$$key=\$_['$key'];";
			$eval .= "unset(\$_);require \"$template_file\";";
			eval($eval);
			return ;//ob_get_clean();
		}

		return self::KC_BROWSER_OK;
	}
