...
class Application_Model_kclib_HttpCache {
  ...
  /** Check if given modification time is newer than client-side one. If not,
    ...
    * @param integer $mtime */
    ...
    static function checkMTime($mtime, $sendHeaders=null) {
        //header("Last-Modified: " . gmdate("D, d M Y H:i:s", $mtime) . " GMT"); moved in view
        $heads = array();
        $heads[] = "Last-Modified: " . gmdate("D, d M Y H:i:s", $mtime) . " GMT";
        ...
        if (is_array($headers) && isset($headers['If-Modified-Since'])) 
        {
            ...
            if ($client_mtime >= $mtime) 
            {
                ...
                $heads[] = 'HTTP/1.1 304 Not Modified';
